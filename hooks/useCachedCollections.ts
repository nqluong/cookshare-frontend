...
export function useCachedCollections({ userId, pageSize = 100 }: UseCachedCollectionsOptions) {
  ...
  try {
    ...
    if (result.data) {
      ...
      // fetchFunction trả về PageResponse<CollectionUserDto> trực tiếp
      // Vậy result.data chính là { content: [...], totalElements, ... }
      if (result.data?.content && Array.isArray(result.data.content)) {
        content = result.data.content;
        console.log('✅ Found collections in result.data.content:', content.length);
      }
      // Fallback: nếu cache lưu theo format khác
      else if (Array.isArray(result.data)) {
        content = result.data;
        console.log('✅ Found collections as array:', content.length);
      }
      else {
